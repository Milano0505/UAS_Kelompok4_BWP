...
function Header({ timeText, greeting, onInventoryClick, money }) {
  return (
...
export default function App() {
  ...
  function calculateLifeScore(state) {
    ...
    const statScore = state.meal + state.sleep + state.hygiene + state.happy; // max 400

    // 2. Activities
    const activityScore = state.activitiesDone * 10; // tiap aktivitas +10
    ...
    const areaScore = state.visitedAreas.length * 20; // max 100 (5 tempat)
